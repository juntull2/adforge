...
with tab3:
    ...
    st.markdown("---")
    st.markdown("### 📌 네이버 크리에이터 스튜디오 채널 영상 업로드 & 상품 스티커 등록 가이드")
    
    col_g1, col_g2 = st.columns(2)
    with col_g1:
        st.markdown("""
        #### 💻 1. 네이버 크리에이터 스튜디오 (공식 채널 업로더):
        1. **[네이버 크리에이터 스튜디오 채널 바로가기](https://creator.tv.naver.com/channel/healthylife1/content/video)** 클릭하여 접속
        2. **`[동영상 업로드]`** 클릭 후 제작하신 MP4 파일 등록
        3. 위에서 복사한 **`📌 24자 추천 제목`** 및 **`📝 200자 본문 설명`**, **`🏷️ 10개 해시태그`** 붙여넣기!
        4. 편집 도구의 **`[🏷️ 스티커]` ➡️ `[쇼핑 스티커]`**에 **`스마트스토어 상품 URL`** 붙여넣기(Ctrl+V)
        """)
    with col_g2:
        st.markdown("""
        #### 📱 2. 모바일 (네이버 앱 클립 업로더):
    ...
    st.markdown("🚀 **[네이버 크리에이터 스튜디오 (healthylife1 채널) 영상 업로드 바로가기 (Click)](https://creator.tv.naver.com/channel/healthylife1/content/video)**")
