...
def save_ad_reference_to_notion(
    ...
    # 프로퍼티 구성 (데이터베이스 컬럼명과 정확히 일치해야 함)
    properties = {
        "광고 카피": {
        ...
        "브랜드": {
            "rich_text": [{"text": {"content": brand}}] if brand else []
        },
        "진행 여부": {
        ...
        "키워드": {
            "rich_text": [{"text": {"content": keyword}}] if keyword else []
        },
    }

    # 선택 필드 None 제거
    properties = {k: v for k, v in properties.items() if v is not None and list(v.values())[0] is not None}
...
def batch_save_to_notion(
    ...
    Args:
        items: [{"ad_copy", "reference_url", "brand", "keyword", "page_name"}, ...]
    Returns:
